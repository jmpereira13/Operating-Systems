...
int inode_create(type nType) {
    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (inode_table[inumber].nodeType == T_NONE) {
            ...
            init_lock(inumber);
            lockInode(inumber, "W");
            //if some other thread has already filled the entry
            if (inode_table[inumber].nodeType != T_NONE){
                unlockInode(inumber);
                continue;
            }

            /* if equals to Tnone follow the code
...
int dir_add_entry(int inumber, int sub_inumber, char *sub_name) {
    ...
    if ((sub_inumber < 0) || (sub_inumber > INODE_TABLE_SIZE) || (inode_table[sub_inumber].nodeType == T_NONE)) {
        if(inode_table[sub_inumber].nodeType==T_NONE)
            printf("%d\n", sub_inumber);
        printf("inode_add_entry: invalid entry inumber   FILHO\n");
        return FAIL;
    }
...
/**
 * Init lock in the individual inodes
 * Input:
 *  - inumber: inumber 
 */
void init_lock(int inumber){
    pthread_rwlock_init(&inode_table[inumber].inodeLock, NULL);
}


/*
 * Lock the rwlock of the given iNode
 * Input:
 *  - inumber: identifier of the iNode
 *  - type: "W"(writing) / "R" (reading)
 */
void lockInode( int idNumber, char *type ){
    /* write lock */
    if(strcmp(type, "W") == 0 ){
        pthread_rwlock_wrlock(&inode_table[idNumber].inodeLock);
    }
    /* read lock*/
    if(strcmp(type, "R") == 0 ){
        pthread_rwlock_rdlock(&inode_table[idNumber].inodeLock);
    }
}


/**
 * Unlock the lock of the given iNode
 * Input:
 *  - inumber: identifier of the iNode
 */
void unlockInode( int idNumber ){
    pthread_rwlock_unlock( &inode_table[idNumber].inodeLock );
}


/**
 * Unlock every inode that is in the array
 * Input
 *  - array: array with the inode locks
 *  - head: head of the array 
 */
void unlockArray(int *array, int *head){
    for(; *head>=0;){
        unlockInode( array[(*head)--]);
    }
    *head=0;
}
